...
\begin{abstract}
   % 풀고자 하는 문제
   We propose the novel pedestrian detection method for crowd scenes which can provide an exact visible appearance of partially occluded pedestrians.
   % 기존의 연구
   Although the state-of-the art detectors based on a single pedestrian have achieved a great performance on moderate scenes, they have failed on severely occluded pedestrians.
   % 기존의 Single pedestrian을 기반으로 하는 detector들은 partially occlude된 pedestrian을 잘 찾지 못하기때문에 이에 대한 해결책으로 기존에 많은 방법들이 제안되었다.
   Thus, a numbers of alternative methods for crowd scene have been proposed, but many of them just modeled the spatial relationship between bounding boxes of each pedestrian to reduce the number of missed pedestrian.
   ...
   % 우리의 방법
   In this paper, we propose the pedestrian detection method associating visible parts according to each pedestrian.
   We formulate this association problem as a Binary Quadratic Programming (BQP) which is able to solve the occlusion reasoning and the detection redundancy problem at once.
...
% The proposed method는 target image에서 탐지된 body part detection (이하 part) set $\mathbf{P}=\{p_1,...,p_K\}$를 input으로 받아 이를 pedestrian별로 associate하여, 최종적으로 pedestrian들의 bounding box set $\mathbf{D}=\{D_1,,...,D_N\}$의 생성을 목적으로 한다.
...
% 각 part $p_i = (b_i, t_i, r_i)$는 bounding box인 $b_i$와 part의 type index $t_i$, 그리고 part filter response인 $r_i$로 정의된다. 
...
% 본 장에서는 제안하는 방법이 어떠한 방식으로 part association 문제를 생성하는지와 함께, 이를 binary quadratic programming로 formation하는 방법에 대해 다룬다.
