// mex_test.cpp : Defines the exported functions for the DLL application.
//
...
{
	/* input:
		- CD1PartCoords(missed) : 4xN double, [0, 0, 0, 0]' for visible parts
		- CD2PartCoords(visible): 4xN double, [0, 0, 0, 0]' for missed parts
		- partOccMinOverlap
		- rescaleForSpeedup
	*/
	// combinations
	...
	// scalar parameters
	double minOverlap = mxGetScalar(prhs[2]);
	double rescale    = mxGetScalar(prhs[3]);
	// for processing
	char *overlapMap = NULL;
	// output
	double numOccludedParts = 0.0;
	...
	{			
		...
		// check overlap
		if (GetOverlapRegionRatio(rectTarget, overlapMap) >= minOverlap)
		{
			numOccludedParts++;
		}		
		mxFree(overlapMap);		
	}
	plhs[0] = mxCreateDoubleScalar(numOccludedParts);
}
